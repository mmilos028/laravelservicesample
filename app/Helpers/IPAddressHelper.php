...
class IPAddressHelper {
	...
	public static function getRealIPAddress(){
        ...
        if(self::testPrivateIP($ip)){
            $ip = self::defaultPublicIPAddress();
        }

		return $ip;
	}

	public static function getDetectedIPAddress(){
		if(!empty($_SERVER['HTTP_CLIENT_IP'])){
			$ip = $_SERVER['HTTP_CLIENT_IP'];
		}
		elseif (!empty($_SERVER['HTTP_X_FORWARDED_FOR'])){
    ...
    public static function defaultPublicIPAddress(){
        return "212.200.99.50";
    }
	...
	public static function testPrivateIP($ip_address){
        ...
        $ip_start = ip2long("127.0.0.0");
        $ip_end = ip2long("127.255.255.255");
        $ip_test = ip2long($ip_address);
        if($ip_test > $ip_start && $ip_test < $ip_end){
            //if is loopback ip address in private range
            return true;
        }

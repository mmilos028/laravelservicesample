...
class CashierServiceMakeTicketsShiftTest extends TestCase
{
    ...
    public function testLogin()
    {
        ...
        $data = array(
            'operation'=> 'login-cashier', //bilo je login-cashier ???? pa nije radilo sa time
    ...
    public function testSaveTicketInformationShift1(){
        $url = getenv('WEB_SERVICE_URL') . '/rest/json';
        $client = new Client(['headers' => [ 'Content-Type' => 'application/json' ]]);
        ...
        $this->assertArrayHasKey('operation', $response_data);
        $this->assertArrayHasKey('session_id', $response_data);
        $this->assertArrayHasKey('list_barcodes', $response_data);
        $this->assertEquals("OK", $response_data['status']);
